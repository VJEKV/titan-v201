...
Преобразование сырых данных в рабочий формат.
"""
...
def calculate_data_completeness(row, required_fields):
    ...
    Returns:
        float: процент заполненности (0-100)
    """
    filled = 0
...
def aggregate_status_history(df):
    """
    Агрегация истории статусов для нового формата выгрузки.
    
    Преобразует множество строк на заказ (по статусам) в одну строку
    с агрегированными метриками.
    
    Args:
        df: DataFrame с историей статусов
        
    Returns:
        pd.DataFrame: агрегированные данные (один заказ = одна строка)
    """
    # Статические поля (берём первое значение)
    static_cols = [
    ...
    # 4. Количество возвратов статусов (КЛЮЧЕВАЯ МЕТРИКА для NEW-10!)
...
def process_data(df_raw):
    ...
    Выполняет:
    - Определение формата выгрузки
    - Агрегацию истории статусов (если нужно)
    - Маппинг колонок
    ...
    Args:
        df_raw: сырой DataFrame из load_file()
    ...
    try:
        ...
        # Превышение длительности (числовой тип!)
    ...
    # ========================================
    # ИЕРАРХИЯ ИЗ СПРАВОЧНИКА tm_structure.json
    # Логика: ЕО → ТМ → иерархия (производство, цех, установка)
    # ========================================
    try:
    ...
    if has_structure:
        # Векторизованное заполнение иерархии
        def get_hierarchy_for_row(row):
            """Получает иерархию для строки по ЕО или ТМ."""
            result = {
                'производство_код': None,
                'производство_название': None,
                'цех_код': None, 
                'цех_название': None,
                'установка_код': None,
                'установка_название': None
            }
            
            # 1. Пробуем по коду ЕО
            eo_code = row.get('EQUNR_Код') or row.get('ЕО')
            if eo_code and str(eo_code) not in ['Н/Д', 'nan', 'None', '']:
                eo_str = str(eo_code).strip()
                # Извлекаем числовой код если есть текст
                if ' ' in eo_str:
                    eo_str = eo_str.split()[0]
                
                # Пробуем с ведущими нулями и без (SAP: 000000101000000468 → 101000000468)
                eo_stripped = eo_str.lstrip('0') or eo_str  # Не допускаем пустую строку
        ...
        # Форматируем с наименованиями: "КОД - Название"
        ...
        # УСТАНОВКА: используем из справочника если есть
        ...
        # ТМ: форматируем как "КОД - Название" если код есть
        ...
        # УСТАНОВКА: также форматируем с кодом если есть
        if 'УСТАНОВКА_Код' in df.columns:
            def format_ust(row):
                ...
                if ust_kod and ust_kod not in ['Н/Д', 'nan', 'None', '']:
                    # Если УСТАНОВКА уже содержит код — не дублируем
        ...
        # Fallback: старая логика парсинга из кода
        if 'ТМ_Код' in df.columns:
